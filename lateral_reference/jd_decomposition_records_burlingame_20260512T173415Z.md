...
**Status:** Updated canonical instance of the per-agency lateral reference file for Burlingame water utility decomposition. Second-spec landing (Water System Operator Trainee) at this advance; first canonical instance of the agency-intake protocol's per-spec workflow applied at verbatim fidelity from primary source. Composed under the working-window discipline: per-spec skeleton → record → rollup staging.
...
**Specs landed at this canonical timestamp:** 2 of 9 (Water Division Manager, Water System Operator Trainee).
...
| JDR_BG_WSO | Water System Operator | `Water_System_Operator_-_September_2024.pdf` | `/DocumentCenter/View/...` |
...
| JDR_BG_WQS | Water Quality Supervisor | `Water_Quality_Supervisor__PDF_.pdf` | `/DocumentCenter/View/...` |
...
**Scope-departure flag.** A `Zoning_Technician__PDF_.pdf` (class code A110, Planning Department, AFSCME 829 Maintenance Unit, 1993, no subsequent revision) was uploaded alongside the second-canonical-instance specs. This spec is **outside the water cohort scope** and is not decomposed in this agency file. Two protocol-relevant observations carried by it:
- A new class-code prefix `A` is in use at Burlingame (current Section 1 schema documents only S and B prefixes).
- The PDF's page footer carries the text "WATER SERVICE & OPERATIONS TECHNCIAN" — explicit template-reuse error from a Water-Division spec; direct evidence that Burlingame HR composes specs from prior-spec templates and that footer-text is not a reliable provenance signal.

Scope decision pending: extend the agency file to multi-cohort (water + planning + others) vs instantiate a separate `jd_decomposition_records_burlingame_planning_<ts>.md`. Held for stakeholder direction.
...
### Ladder-progression update (Operations ladder, WSOT row at verbatim fidelity)

| Stratum | Spec | Allocative % | Fidelity |
|---|---|---|---|
| trainee | WSOT | 0.00% (Core-weighted); 0.00% (unweighted) | verbatim (this file) |
| journey | WSO | 6.2% (recovery-from-conversation) | pending verbatim re-decomposition |
...
WSOT row at verbatim fidelity confirms the recovery-from-conversation value of 0.0% Allocative. The cone-hypothesis floor for the Operations ladder is therefore established at full canonical fidelity. Subsequent verbatim re-decompositions of WSO, WOS, and (eventually) WDM at full source fidelity will incrementally retire the recovery-from-conversation caveats on the upper ladder rows.

Monotonic Allocative rise from 0% → ~6% → (gap) → ~39% → ~44% remains the working pattern; the WSO row will be the next high-leverage validation when that spec is decomposed in this session sequence.
...
*Composed 2026-05-12 17:34 UTC at second canonical instance (Water System Operator Trainee landing). Prior canonical at 2026-05-12 17:10 UTC (first per-agency-file instance, Water Division Manager from conversation-history recovery). This instance is the first verbatim-source decomposition in the agency file; subsequent appends will advance the canonical timestamp at real composition time per protocol Section 8 discipline. **Recovery provenance summary at this instance:** WDM record carries the recovery-from-conversation provenance from the prior instance (partial fidelity, 3 functions reconstructed); WSOT record is fully verbatim from primary-source PDF. **Timestamp note:** The project chain's most recent handoffs carry later stamps (Thread 11 close at `20260516T080000Z`), making this file's timestamp non-monotonic vs the chain. The non-monotonicity is preserved deliberately: real composition time is the reconstruction anchor; the apparent regression reflects a real cross-session date discrepancy between this session's clock and prior sessions' clocks.*
